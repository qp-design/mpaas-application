	...
	<view class="sales-back">
			...
			<view class="sales-message">
				...
				<view class="sales-desc">
					...
					<view class="sales-import">
						...
						<view class="sales-num" v-if="goodDetail.goodsLogmoney">邮费：{{ goodDetail.goodsLogmoney}}</view>
					</view>
				</view>
			</view>
			<view class="back-goods-num">
				<text class="title">申请数量</text>
				<view class="step">{{refundPolicy.refundGoodsNum}}</view>
		...
		<view class="back-results">
			...
			<view class="back-results-middle">
				<u-input class="text-box" v-model="refundPolicy.refundMeo" :clearable="false" type="textarea" placeholder="请描述申请售后服务的具体原因。"
				 height="172" maxlength="500" />
			...
			<view class="back-results-bottom">
				<view class="upload-box">
					...
					<view class="img-item last-item" :class="isclick?'':'pointer'" v-show="upImg.length < 9" @click="chooseImage">
						<i class="iconfont">&#xe620;</i>
		...
		<!-- 提交按钮 -->
		<view class="submit">
			<view @click="submitAfterSales">提交</view>
		</view>

		<!-- 申请原因弹出框 -->
		<view class="detail--reasonsApplication">
			<u-popup v-model="reasonsApplicationShow" mode="bottom" width="100%" height="40%" border-radius="24" closeable>
				<view class="detail--reasonsApplication--main">
					<view class="detail--reasonsApplication--main--position">申请原因</view>
					<u-radio-group active-color="red" v-model="refundPolicy.refundEx" @change="groupChange">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			// #endif
			// #ifdef H5
			this.goodDetail = JSON.parse(this.$percent2percent25(options.detail))
			// #endif
			this.getRefCause();
			// 商品id
		...
		watch: {
			...
			// #ifdef H5
			reasonsApplicationShow(newValue, oldValue) {
				if (!newValue) {
		...
		methods: {
			groupChange(){
				this.reasonsApplicationShow = false
			},
			// 选择图片
			chooseImage(e) {
				var that = this;
				uni.chooseImage({
					...
					success(res) {
						...
						res.tempFilePaths.forEach((val, i) => {
							that.lenum ++
							that.upLoad(val,res.tempFilePaths.length); //上传操作
						});
					},
					fail: function() {
			...
			submitAfterSales() {
				...
				let query = {
					...
					refundMoney: Number(this.goodDetail.contractGoodsMoney) + Number(this.goodDetail.goodsLogmoney || 0),
				...
				this.upImg.forEach(item=>{
					...
					query.ocRefundFileDomainList.push(img)
				})
				let arr = []
				arr.push({
					...
					// 退款金额
					refundGoodsAmt: this.refundPolicy.refundGoodsNum == this.goodDetail.goodsCamount ? this.goodDetail.contractGoodsMoney :
						(this.goodDetail.contractGoodsPrice * this.refundPolicy.refundGoodsNum).toFixed(2),
				})

				query.ocRefundGoodsBeanList = arr
				http.post(refund.saveRefund, {
					params: JSON.stringify(query)
				}).then(res => {
					if (res.success) {
						this.$message.alert('申请成功')
						setTimeout(() => {
							//delta=1指从退款页面跳转，!=1指个人中心跳转
							this.$routers.push('afterSales/index', {delta: 1})
						}, 800)
					} else {
