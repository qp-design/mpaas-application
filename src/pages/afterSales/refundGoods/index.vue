	...
	<view class="sales-back">
		...
		<view class="back-goods">
			...
			<view class="back-goods-num">
				...
				<view class="step"><!-- <u-number-box :min="1" :max="goodDetail.goodsNum" :size="24" v-model="refundPolicy.refundGoodsNum"></u-number-box> -->{{refundPolicy.refundGoodsNum}}</view>
			</view>
		</view>
		...
		<view class="detail--reasonsApplication">
			<u-popup v-model="reasonsApplicationShow" mode="bottom" width="100%" height="40%" border-radius="24" :closeable="true">
				<view class="detail--reasonsApplication--main">
					...
					<u-radio-group active-color="red" v-model="refundPolicy.refundEx">
	...
	methods: {
		...
		submitAfterSales() {
			...
			arr.push({
				...
				// 退款金额
				refundGoodsAmt: +(this.refundPolicy.refundGoodsNum == this.goodDetail.goodsCamount ? this.goodDetail.contractGoodsMoney : (+(this.goodDetail.contractGoodsMoney/this.goodDetail.goodsCamount) * +(this.refundPolicy.refundGoodsNum)).toFixed(2)),
			})
