<template>
	<div class="goods-list">
		...
		<navBar inputPlaceholder="输入商品或店铺名称" @getNavBarHeight="getNavBarHeight"></navBar>
		<view class="header" v-show="params.bizType">

			<view class="header-other" v-bind:style="{ top: topDistance + 'px' }">
				<ul class="nav-left top_tabs">
					<li v-for="(item, index) in tabsList" :key="index" @click="changeTabs(index)">
						<view class="ttitle" :style="{ fontWeight: actTab === titIndex ? '500' : '' }">
							{{ item }}
						</view>
						<view class="line" v-if="actTab === index"></view>
					</li>
				</ul>
				<div class="nav" v-if="actTab==0">
					<ul class="nav-left">
						<li v-for="(titles, titIndex) in titleList" :key="titIndex" @click="titleClick(titIndex)">
							...
							<!-- #endif -->
							<!-- #ifdef H5 -->
							<view class="line" v-show="current === titIndex"></view>
							<!-- #endif -->
						</li>
						<li>
		...
		<keep-alive v-if="actTab==0">
			<!-- 筛选弹窗 -->
			<mPopup v-model="isShowScreen" :padd_Top="topDistance" :scrollTopBox="scrollTopBox" mode="right" width="640"
			 @scroll="scroll">
			...
			<!-- 列表 -->
			<div :class="['list',params.bizType ? 'active' : '']" v-if="items.length !== 0&& !NoResult">
				<!-- #ifdef LPHTEST -->
				<verticalList :recomList="items" :status="lastPageLine" :state="1" v-if="isShowCom"></verticalList>
				<!-- #endif -->
				<!-- #ifdef H5||TEST  -->
		...
		<!-- 列表2 -->
		<keep-alive v-if="actTab==1">
			<shopList ref="shopList" :shopdeName="params.searchParam" :page.sync="page" :rows="rows"></shopList>
		</keep-alive>
	</div>
</template>
<script>
	...
	export default {
		...
		watch:{
			...
			lastPageLine(newValue, oldValue) {
				if (newValue === 'nomore' || newValue === 'loadmore') {
					let dom = document.querySelectorAll('.u-loading');
					if(dom.length>1){
					for (var i = 1; i < dom.length; i++) {
						dom[i].remove();
					}
					}
					let dom1 = document.querySelectorAll('.u-load-more-wrap');
					if(dom1.length>1){
						for (var i = 1; i < dom1.length; i++) {
							dom1[i].remove();
						}
					}
				}
		...
		onReachBottom() {
			...
			if(this.actTab==0){
				...
				if(this.$refs.shopList.shopList.length==this.rows){
					this.page++
				}else{
					return
				}
		...
		methods: {
			...
			commonMounted() {
				...
				this.params.goodsType='00,40'
				
				this.lastPageLine = 'loading';
				http.get(search, this.params).then(res => {
					if (res && res.rows && res.rows.length > 0) {
						this.items = res.rows;
						//.filter(itemk=> itemk.goodsType != '24'&&itemk.goodsType != '26'   )
