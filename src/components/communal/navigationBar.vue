<template>
    <div class="comp-navbar">
        ...
        <!-- 导航栏主体 -->
        <cover-view class="navbar" :style="{height: navBarHeight + 'px',backgroundColor:navBackgroundColor,marginBottom:navBarHeight + 'px'}">
            <!-- 状态栏 -->
    ...
    beforeMount() {
        ...
        wx.getSystemInfo({
            success(system) {
                ...
                self.$storage.set('navBarHeight',self.navBarHeight)
            }
        });
    },
    mounted() {
    ...
    methods: {
        backClick() {
            if (getCurrentPages().length == 1) {
                // 打开分享卡片无法回退
				this.$routers.replace(this.homePath)
            } else {
				this.$routers.back()
            }
        },
